...
class Player(pygame.sprite.Sprite):
    ...
    def collide_bullet(self):
        ...
        if hits:
            ...
            hits[0].kill()
        boss_hits = pygame.sprite.spritecollide(self, self.game.boss_bullets, False)
        if boss_hits:
            self.get_dmg(boss_hits[0].dmg)
            boss_hits[0].kill()
    def collide_blocks(self, dir):
        hits = pygame.sprite.spritecollide(self, self.game.blocks, False)
        if hits:
    ...
    def collide_blocks(self, dir):
        ...
        if hits:
            if (dir == "x"):
                ...
                if self.x_change == 0:
                    self.rect.right = hits[0].rect.left-self.width/2
                    self.rect.left = hits[0].rect.right+self.width/2
            if (dir == "y"):
                if self.y_change > 0:
                    self.rect.bottom = hits[0].rect.top
                ...
                if self.y_change == 0:
                    self.rect.bottom = hits[0].rect.top-self.height/2
                    self.rect.top = hits[0].rect.bottom+self.height/2
class Block(pygame.sprite.Sprite):
    def __init__(self, game, x, y, mapx, mapy):
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, game, heading, rad, dmg, rad_offset, speed, owner):
        ...
        if isinstance(owner, Boss):
            self.groups = self.game.all_sprites, self.game.boss_bullets
        else:
            self.groups = self.game.all_sprites, self.game.bullets
...
class MyMap(pygame.sprite.Sprite): 
    ...
    def update_phase(self):
    # #get num enemy in self.enemies
    #     num_of_enemies = len(self.game.enemies.sprites())
    #     count_boss=0
    ...
    #         if not self.boss_appreared:
    #             boss_pos=random.sample(self.available_pos, 1)
                
    #             boss = Boss(self.game, 7, 7, self.rect.x-32, self.rect.y-32, self)
    #             self.enemies.add(boss)
    #             count_boss+=1
    #             print("Boss appeared")
                
    #         self.boss_appreared = True
    #         self.clear = True 
    #     if count_boss>0:
    #         self.clear = True
    #         self.boss_appreared=True
    #     if  self.boss_appreared == True:
    #         self.clear = True
    #         self.boss_appreared=True
        ...
        if num_enemies == 0 and self.current_phase == self.max_phase:
            print("All enemies are dead")
            self.clear = True
        if num_boss == 0 and self.current_phase == self.max_phase and self.clear == True:
            print("All boss are dead")
            self.clear = True
        if len(self.enemies.sprites())==0 and self.current_phase < self.max_phase:
                self.current_phase += 1
                #chon 5 vi tri random de tao enemy
